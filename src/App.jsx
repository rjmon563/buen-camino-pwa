...
.selector-container{position:fixed; top:20px; left:20px; z-index:2000; width:220px;}
...
.stage-info-box { background: rgba(0,0,0,0.85); color: var(--orange); border: 2px solid var(--orange); border-top: 0; padding: 6px 10px; font-size: 10px; font-weight: 800; border-radius: 0 0 8px 8px; }

.bottom-console { position: fixed; bottom: 0; left: 0; right: 0; background: #0a0a0a; border-top: 3px solid var(--yellow); display: flex; justify-content: space-around; align-items: center; padding: 12px 10px 25px 10px; z-index: 3000; }
...
.emergency-popover { position: absolute; bottom: 95px; left: 10px; display: flex; flex-direction: column; gap: 8px; width: 220px; z-index: 4000; }
.pop-btn { padding: 15px; border-radius: 12px; border: 2px solid white; color: white; font-weight: 900; display: flex; align-items: center; gap: 10px; box-shadow: 0 4px 15px rgba(0,0,0,0.5); }
...
export default function App() {
  const [activeStage, setActiveStage] = useState(() => JSON.parse(localStorage.getItem('stage_v49')) || STAGES[0]);
  const [steps, setSteps] = useState(() => parseInt(localStorage.getItem('steps_v49')) || 0);
  ...
  useEffect(() => {
    localStorage.setItem('steps_v49', steps);
    localStorage.setItem('stage_v49', JSON.stringify(activeStage));
  }, [steps, activeStage]);
  ...
  useEffect(() => {
    ...
    const watchId = navigator.geolocation.watchPosition(
      ...
      null, { enableHighAccuracy: !batterySave, timeout: 10000, maximumAge: 0 }
    );
    ...
    <div className="h-screen w-screen bg-black overflow-hidden flex flex-col">
      {showOverlay && (
        <div className="sensor-overlay">
          ...
          <button onClick={() => setShowOverlay(false)} className="px-12 py-5 bg-cyan-500 text-black font-black rounded-2xl">FIX & START v49</button>
        </div>
      )}

      {/* SELECTOR CORREGIDO: NO TAPA NADA */}
      <div className="selector-container">
        <select value={activeStage.id} onChange={(e) => {
          ...
          {STAGES.map(s => <option key={s.id} value={s.id}>ETAPA {s.id}</option>)}
        </select>
      ...
      <div className="tactical-stats">
        <div className="text-[8px] text-cyan-400 font-bold uppercase">Status_Live</div>
        ...
        <div className="relative">
          {sosMenu && (
            <div className="emergency-popover">
              <button onClick={() => window.open('tel:112')} className="pop-btn bg-red-600"><Phone size={20}/> 112 EMERGENCIA</button>
              <button onClick={() => window.open('tel:062')} className="pop-btn bg-blue-800"><ShieldAlert size={20}/> 062 GUARDIA CIVIL</button>
            </div>
          )}
          <button onClick={() => setSosMenu(!sosMenu)} className="btn-ui btn-sos">
            <AlertTriangle size={30}/><span className="text-[11px] font-black">SOS</span>
          </button>
        </div>

        <button onClick={() => window.open(`https://wa.me/?text=Reporte: ${activeStage.name}. Faltan ${distanceToTarget.toFixed(2)} km.`)} className="btn-ui btn-wa">
          <MessageCircle size={30}/><span className="font-black text-[9px]">REPORT</span>
        </button>

        <button onClick={() => setIsTracking(!isTracking)} className={`btn-ui btn-mira ${isTracking ? 'bg-yellow-400' : 'opacity-80'}`}>
