...
import { 
  ...
  Crosshair, PhoneCall, Navigation, Map as MapIcon, Play
} from 'lucide-react';
...
const STYLES = `
  ...
  .pedometer-dashboard { 
    ...
    padding: 15px; border-bottom: 2px solid var(--yellow); 
    display: flex; justify-content: space-between; align-items: center; 
  }

  .stage-list-container { flex: 1; overflow-y: auto; padding-bottom: 30px; }
  
  .stage-card { 
    border-left: 4px solid #333; background: #0a0a0a; 
    border-bottom: 1px solid #222; padding: 12px; cursor: pointer; 
    transition: all 0.2s;
  }
  ...
  .scope-pulse { 
    position: absolute; width: 30px; height: 30px; 
    border: 2px solid var(--red); border-radius: 50%; 
    animation: scopePulse 2s infinite; 
  }
  ...
  .btn-sos-red-circle { 
    ...
    box-shadow: 0 0 25px var(--red); cursor: pointer; 
  }
  ...
  .distance-tag { 
    ...
    border: 1px solid #2563eb; color: #60a5fa; 
    padding: 4px 8px; border-radius: 4px; 
    font-size: 10px; font-weight: 800; 
    margin-top: 5px; display: inline-flex; 
...
export default function App() {
  ...
  const handleMotion = (e) => {
    ...
    if (Math.abs(force - prevForce) > 13 && !stepDebounce.current) {
      setSteps(s => s + 1);
  ...
  const sharePosition = () => {
    ...
    const msg = `POSICIÓN TÁCTICA: https://www.google.com/maps?q=${userPos[0]},${userPos[1]}`;
    ...
    <div className="flex flex-col h-screen w-screen bg-black overflow-hidden">
      {showOverlay && (
        <div className="sensor-overlay">
          <Zap className="text-yellow-500 mb-6" size={80} fill="currentColor" />
          <h2 className="text-yellow-500 font-black text-2xl mb-2 uppercase italic">Santiago Tactical</h2>
          <p className="text-white/40 text-[10px] mb-8 uppercase tracking-[0.2em]">Sensor Authorization Required</p>
      ...
      <header className="h-16 bg-black border-b-2 border-yellow-500 flex items-center justify-between px-4 z-[6000]">
        <div className="flex items-center gap-2">
          <Zap className="text-yellow-500" size={24} fill="currentColor" />
          <div>
            <h1 className="text-yellow-500 font-black text-xs m-0 uppercase italic tracking-tight">Santiago Tactical v25.0</h1>
            <p className="text-[7px] text-white/30 m-0 font-bold uppercase tracking-widest">Secure Military Feed</p>
          </div>
        </div>
        <div className="flex items-center gap-3 bg-red-600/10 px-3 py-1 border border-red-600/30 rounded">
          ...
          <span className="text-red-600 text-[9px] font-black uppercase">LIVE GPS FEED</span>
        </div>
      </header>
      ...
      <div className="flex flex-1 overflow-hidden relative">
        <aside className="sidebar-tactical">
          <div className="pedometer-dashboard">
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-xl font-black text-white leading-none">{steps.toLocaleString()}</span>
              </div>
            </div>
            <div className="flex flex-col items-end">
              <span className="text-[8px] text-white/40 font-black uppercase">DISTANCE</span>
              <span className="text-sm font-black text-yellow-500">{(steps * 0.00075).toFixed(2)} KM</span>
            </div>
          </div>
          ...
          <div className="p-2 bg-yellow-500 text-black text-[9px] font-black uppercase text-center flex items-center justify-center gap-2">
            <MapIcon size={12} /> 33 OBJETIVOS CARGADOS
          </div>
          ...
          <div className="stage-list-container">
            {STAGES.map((s) => (
              <div 
                key={s.id} 
                onClick={() => { setActiveStage(s); setIsTracking(false); }} 
                className={`stage-card ${activeStage.id === s.id ? 'active' : ''}`}
              >
                <div className="flex justify-between items-center mb-1">
                  <span className="text-[8px] font-black text-white/30 uppercase">OBJ: {String(s.id).padStart(2, '0')}</span>
                ...
                {activeStage.id === s.id && (
                  <div className="mt-2 space-y-2">
                    ...
                    <p className="text-[9px] text-white/60 leading-tight m-0 uppercase font-bold italic">{s.tips}</p>
                  </div>
                )}
              </div>
            ))}
        ...
        <main className="flex-1 relative bg-black">
          <div className="floating-controls">
            <button onClick={sharePosition} className="btn-gps-wa">
            ...
            <button onClick={() => window.location.href="tel:112"} className="btn-emergency-pill">
              ...
              <div>112 EMERGENCIA<br/><span style={{fontSize:'8px', opacity:0.7}}>GUARDIA CIVIL: 062</span></div>
            </button>

            <button 
              onClick={() => setIsTracking(!isTracking)} 
              className={`btn-tactical ${isTracking ? 'bg-yellow-500' : 'bg-gray-800'}`}
            >
              <Crosshair size={28} color={isTracking ? "black" : "white"} />
            </button>
          </div>

          <MapContainer center={activeStage.coords} zoom={15} zoomControl={false} style={{height: "100%", width: "100%"}}>
            <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
            <Polyline positions={STAGES.map(s => s.coords)} color="#00e5ff" weight={4} opacity={0.6} dashArray="5, 10" />
            {userPos && (
