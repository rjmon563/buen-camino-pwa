...
import {
  ...
  Eye, EyeOff, AlertTriangle, Phone, ShieldAlert, Mountain
} from 'lucide-react';
...
.tactical-stats{position:fixed; top:20px; right:20px; z-index:1000; background:rgba(0,0,0,0.9); border:2px solid var(--cyan); padding:10px; border-radius:8px; min-width:140px; text-align:right;}
.selector-container{position:fixed; top:20px; left:20px; z-index:1100; width:220px;}
select { background: #111; color: var(--yellow); border: 2px solid var(--yellow); padding: 12px; font-family: 'JetBrains Mono'; border-radius: 6px; width: 100%; font-size: 10px; font-weight: 800; text-transform: uppercase; }
...
.emergency-popover { position: absolute; bottom: 95px; left: 10px; display: flex; flex-direction: column; gap: 8px; width: 240px; z-index: 3000; }
.lock-indicator { position: absolute; top: -5px; background: red; color: white; font-size: 8px; padding: 2px 4px; border-radius: 4px; font-weight: 800; animation: blink 1s infinite; }
...
/* ===================== DATA: ETAPAS CON DESNIVEL Y DIFICULTAD ===================== */
const STAGES = [
  ...
  { id:4, name:"Pamplona - Puente la Reina", coords:[42.6719,-1.8139], dist:24.0, dPlus:450, diff:"Media" },
  { id:5, name:"Puente la Reina - Estella", coords:[42.6715,-2.0315], dist:22.6, dPlus:350, diff:"Media" },
  ...
  { id:23, name:"Foncebadón - Ponferrada", coords:[42.5455,-6.5936], dist:26.8, dPlus:150, diff:"Alta (Bajada)" },
  ...
  // Nota: He incluido las más críticas, puedes expandir el array con el mismo formato.
];
...
export default function App() {
  const [activeStage, setActiveStage] = useState(() => JSON.parse(localStorage.getItem('stage_v48')) || STAGES[0]);
  const [steps, setSteps] = useState(() => parseInt(localStorage.getItem('steps_v48')) || 0);
  ...
  useEffect(() => {
    localStorage.setItem('steps_v48', steps);
    localStorage.setItem('stage_v48', JSON.stringify(activeStage));
  }, [steps, activeStage]);
  ...
  useEffect(() => {
    ...
    const watchId = navigator.geolocation.watchPosition(
      ...
      null,
      { enableHighAccuracy: !batterySave, timeout: 5000, maximumAge: 0 }
    );
    ...
    <div className="h-screen w-screen bg-black overflow-hidden flex flex-col">
      {showOverlay && (
        <div className="sensor-overlay">
          ...
          <button onClick={() => setShowOverlay(false)} className="px-12 py-5 bg-cyan-500 text-black font-black rounded-2xl">INICIAR CONSOLA v48</button>
        </div>
      )}
        ...
        <select value={activeStage.id} onChange={(e) => {
          ...
          {STAGES.map(s => (
            <option key={s.id} value={s.id}>
              {s.id}. {s.dist}km | +{s.dPlus}m | {s.diff}
            </option>
          ))}
        </select>
        <div className="bg-black/80 text-[9px] p-2 border-x-2 border-b-2 border-yellow-500 text-yellow-500 font-bold">
          {activeStage.name}
        </div>
      </div>
      ...
      <div className="tactical-stats">
        ...
        <div className="text-sm font-bold text-white mt-1 flex items-center justify-end gap-1">
           {userPos ? distanceToTarget.toFixed(2) : "0.00"} KM
        </div>
        <div className="text-[9px] text-orange-500 font-bold mt-1">OBJ: {activeStage.diff}</div>
      </div>
      ...
      <div className="bottom-console">
        <div className="relative">
          {sosMenu && (
            <div className="emergency-popover">
              <button onClick={() => window.open('tel:112')} className="p-4 bg-red-600 text-white rounded-xl border-2 border-white flex items-center gap-3 font-bold shadow-lg"><Phone size={20}/> 112</button>
              <button onClick={() => window.open(`https://wa.me/?text=SOS: http://maps.google.com/?q=${userPos?.[0]},${userPos?.[1]}`)} className="p-4 bg-green-600 text-white rounded-xl border-2 border-white flex items-center gap-3 font-bold shadow-lg"><MessageCircle size={20}/> WA SOS</button>
            </div>
          )}
        ...
        <button onClick={() => window.open(`https://wa.me/?text=REPORTE: ${activeStage.name} - ${steps} pasos. KM RESTANTES: ${distanceToTarget.toFixed(2)}`)} className="btn-ui btn-wa">
          <MessageCircle size={30}/><span className="font-black text-[9px]">REPORTE</span>
        </button>

        <button onClick={() => setIsTracking(!isTracking)} className={`btn-ui btn-mira ${isTracking ? 'scale-110 border-black' : 'opacity-80'}`}>
          {isTracking && <span className="lock-indicator">LOCK ON</span>}
          <Crosshair size={32}/>
        </button>
